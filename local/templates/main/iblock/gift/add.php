...
if(CModule::IncludeModule("form")){      
    ...
    if($arValues['product_id']){
        $arValues['form_text_12'] = $arValues['product_id'];
        $arValues['form_textarea_13'] = GetProductInfo($arValues['product_id']);
    }
    $error = CForm::Check($_REQUEST['WEB_FORM_ID'], $arValues);    
    if(!$error){   
        if($RESULT_ID = CFormResult::Add($_REQUEST['WEB_FORM_ID'], $arValues)){
            CFormResult::Mail($RESULT_ID, 23);
            CFormResult::Mail($RESULT_ID, 26);
